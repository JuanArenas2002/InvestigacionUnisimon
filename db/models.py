...
from db.models_base import Base, SourceRecordMixin  # noqa: F401 (re-exportado)
...
class AuthorAuditLog(Base):
    ...
    # Snapshot antes/después del cambio
    ...
    # Solo los campos que cambiaron: {campo: {before: v, after: v}}
...
class ResearcherCredential(Base):
    ...
    __table_args__ = (
        ...
        # NOTA: El constraint único condicional (WHERE is_active = true) se crea
        # via migración SQL, no via SQLAlchemy (que no tiene soporte nativo)
    )
...
# =============================================================
# Código existente que importa SOURCE_MODELS / SOURCE_TABLE_NAMES
# directamente continúa funcionando sin cambios.
...
# (ej: from db.models import OpenalexRecord)
