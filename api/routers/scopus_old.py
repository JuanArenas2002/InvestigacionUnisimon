...
SOURCE = "scopus"  # Constante para filtros
...
def scopus_insights(db: Session = Depends(get_db)):
    ...
    - **records**: Conteos de registros Scopus por estado.
    - **coverage**: Cobertura de Scopus sobre el inventario canónico.
    - **field_contributions**: Qué campos del registro canónico fueron aportados por Scopus.
    - **authors**: Cuántos autores tienen Scopus Author ID.
    - **citations**: Métricas de citas donde Scopus es la fuente.
    - **top_journals**: Revistas más frecuentes en registros Scopus.
    - **year_distribution**: Registros Scopus por año de publicación.
    - **enrichment_samples**: Ejemplos de publicaciones que Scopus completó.
    """

    # ── 1. Registros Scopus por estado ───────────────────────
    records = _build_record_summary(db)

    # ── 2. Cobertura vs inventario total ─────────────────────
    coverage = _build_coverage(db)

    # ── 3. Contribución de campos (field_provenance) ─────────
    field_contributions = _build_field_contributions(db)

    # ── 4. Autores con Scopus ID ─────────────────────────────
...
def list_scopus_records(
    ...
    """Lista paginada de registros externos de Scopus con filtros."""
    q = db.query(ExternalRecord).filter(ExternalRecord.source_name == SOURCE)
    ...
    if search:
        ...
        q = q.filter(
            or_(
                ExternalRecord.title.ilike(term),
                ExternalRecord.doi.ilike(term),
            )
        )
    if year:
        q = q.filter(ExternalRecord.publication_year == year)
    if found_only:
        q = q.filter(
            ~ExternalRecord.source_id.like("not-found-%")
        )
...
def get_scopus_record(record_id: int, db: Session = Depends(get_db)):
    ...
    if er.source_name != SOURCE:
        raise HTTPException(400, f"El registro {record_id} no es de Scopus (es de {er.source_name})")
...
def scopus_not_found(
    ...
    Son los registros placeholder con source_id='not-found-{doi}'.
    """
    q = (
        db.query(ExternalRecord)
        .filter(
            ExternalRecord.source_name == SOURCE,
            ExternalRecord.source_id.like("not-found-%"),
        )
    )
...
# ══════════════════════════════════════════════════════════════
# GET /scopus/enriched-fields — Publicaciones cuyo campo fue aportado por Scopus
# ══════════════════════════════════════════════════════════════
...
def scopus_enriched_by_field(
    ...
    """
    # Usar operador JSONB ->> para filtrar
    q = (
...
def _build_record_summary(db: Session) -> ScopusRecordSummary:
    ...
    total = db.query(func.count(ExternalRecord.id)).filter(
        ExternalRecord.source_name == SOURCE
    ).scalar() or 0

    statuses = (
        db.query(ExternalRecord.status, func.count(ExternalRecord.id))
        .filter(ExternalRecord.source_name == SOURCE)
        .group_by(ExternalRecord.status)
        .all()
    )
    status_map = {s: c for s, c in statuses}

    not_found = db.query(func.count(ExternalRecord.id)).filter(
        ExternalRecord.source_name == SOURCE,
        ExternalRecord.source_id.like("not-found-%"),
    ).scalar() or 0
...
def _build_coverage(db: Session) -> ScopusCoverageVsTotal:
    ...
    # Canónicas con al menos 1 registro Scopus real (no placeholder)
    with_scopus = (
        db.query(func.count(func.distinct(ExternalRecord.canonical_publication_id)))
        .filter(
            ExternalRecord.source_name == SOURCE,
    ...
    # Canónicas donde Scopus es la ÚNICA fuente
    only_scopus = (
...
def _build_citation_stats(db: Session) -> ScopusCitationStats:
    ...
    # Publicaciones cuyo citation_count proviene de Scopus
    q = (
        db.query(CanonicalPublication)
        .filter(
            CanonicalPublication.field_provenance.isnot(None),
...
def _build_top_journals(db: Session, limit: int = 20) -> List[ScopusTopJournal]:
    """Revistas más frecuentes en registros Scopus (del raw_data)."""
    rows = (
        db.query(
            text("raw_data->>'prism:publicationName' AS journal"),
            func.count(ExternalRecord.id),
        )
        .filter(
            ExternalRecord.source_name == SOURCE,
            ~ExternalRecord.source_id.like("not-found-%"),
            text("raw_data->>'prism:publicationName' IS NOT NULL"),
        )
        .group_by(text("raw_data->>'prism:publicationName'"))
        .order_by(func.count(ExternalRecord.id).desc())
        .limit(limit)
        .all()
    )
...
def _build_year_distribution(db: Session) -> List[ScopusYearDistribution]:
    ...
    rows = (
        ...
        .filter(
            ExternalRecord.source_name == SOURCE,
            ExternalRecord.publication_year.isnot(None),
            ~ExternalRecord.source_id.like("not-found-%"),
        )
        .group_by(ExternalRecord.publication_year)
        .order_by(ExternalRecord.publication_year)
        .all()
    )
